...
class Model(nn.Module):
	def __init__(self, cfg='yolov5s.yaml', ch=3, ch2=3, nc=None,
		...
		self.model, self.save, self.backbone1depth = parse_model(deepcopy(self.yaml), ch=[ch],
		...
		if isinstance(m, Detect):
			s = 640  # 2x min stride    设置实例化的输入尺寸
			m.inplace = self.inplace
			x = self.forward(torch.zeros(1, ch, s, s), torch.zeros(1, ch2, s, s))

			m.stride = torch.tensor(
				[s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s), torch.zeros(1, ch2, s, s))])  # forward
	...
	def _forward_once(self, x, profile=False, visualize=False):
		...
		for i in range(x.size(0)):
			n = cv2.cvtColor(x[i, ...].permute(1, 2, 0).cpu().numpy().astype('float32'), cv2.COLOR_RGB2YCrCb)
			n = n[numpy.newaxis, ...]
			mm.append(n)
		...
		size1 = x.size(2)
		size2 = x.size(3)
		ycbcr_image = m2.reshape(num_batchsize, 3, size1 // 8, 8, size2 // 8, 8).permute(0, 2, 4, 1, 3, 5)
		...
		if x.dtype==torch.float16:
			x1=ycbcr_image.half().to(x.device)
		if x.dtype == torch.float32:
			x1 = ycbcr_image.to(x.device)
		for m in self.model:
			# print(m.i)
			if m.i < self.backbone1depth:
				if m.i == 1:
					m.fea_size = [size1 // 8,size2 // 8]
				if m.i==2:
					m.fea_size=[size1//16,size2//16]
					x1=y[m.f]
				if m.i==3:
					m.fea_size = [size1 // 32,size2 //32]
					x1=y[m.f]
				x1 = m(x1)
				x2 = x2
...
def parse_model(d, ch, ch2):  # model_dict, input_channels(3)
	...
	# 1、以下是读取配置dict里的参数
	# ————————————————————————————————————————————————————————————————
	anchors, nc, gd, gw, backbone1depth = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple'], d[
		'backbone1depth']
	...
	for i, (f, n, m, args) in enumerate(d['backbone1'] + d['backbone2'] + d['head']):  # 遍历backbone和head的每一层
		...
		if i < backbone1depth:
			if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
					 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost,SpectConv ]:
				c1, c2 = ch[f], args[0]
				if c2 != no:
			...
			if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
					 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost,SpectConv,CBAM]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='../models/improve/yolo5s_fre_cbam.yaml', help='model.yaml')
    ...
    model = Model(opt.cfg).to(device)             #此句需要取消注释

    mm=[]
    for i in range(im.size(0)) :
    	n=cv2.cvtColor(im[i,...].permute(1,2,0).numpy(),cv2.COLOR_RGB2YCrCb)
    	n=n[numpy.newaxis,...]
    	mm.append(n)
    m1=numpy.concatenate(mm)
    m1=torch.from_numpy(m1)
    m2=m1.permute(0,3,1,2)
    print(m2.shape)
    num_batchsize = im.size(0)
    size = im.size(3)
    ycbcr_image = m2.reshape(num_batchsize, 3, size // 8, 8, size // 8, 8).permute(0, 2, 4, 1, 3, 5)
    ycbcr_image = DCT.dct_2d(ycbcr_image, norm='ortho')
