...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # self.dyhead = nn.Sequential(*[DyHeadBlock(ch[0]) for i in range(3)])     #dyhead block's times  2->6
...
class BaseModel(nn.Module):
    ...
    #         else:
    #             if m.f != -1:  # if not from previous layer
    #                 x2 = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
    #             if profile:
    #                 self._profile_one_layer(m, x, dt)
    #             x2 = m(x2)  # run
...
#     #                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, SE, C3SE, CBAMC3,
#     #                                     C3_DCN, SwinV2_CSPB,DeformableConv2d,DenseConv,stem, MBConvBlock
#     #                                  }:
#     #             c1, c2 = ch[f], args[0]
#     #             if c2 != no:  # if not output
#     #                 c2 = make_divisible(c2 * gw, 8)     #改为3？
#     #
#     #             args = [c1, c2, *args[1:]]
#     #     else :
#     #         # if m in {
...
#     #         #
#     #         #     args = [c1, c2, *args[1:]]
#     #         #     if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3SE, CBAMC3, SwinV2_CSPB, C3_DCN}:
...
#     #         #     args = [c1, c2]
#     #         #
#     #         # elif m is SwinTransformer_Layer:
#     #         #     args=args
#     #         #
