    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                tobj[b, a, gj, gi] = iou.reshape((len(iou)))  # iou ratio b.reshape((1,3))
...
class ComputeLoss_class:
    ...
    def __call__(self, p, pre_css, targets, css_label):  # predictions, targets
        lcls = torch.zeros(1, device=self.device)  # class loss
        lbox = torch.zeros(1, device=self.device)  # box loss
        lobj = torch.zeros(1, device=self.device)  # object loss
        tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets
        ...
        lcls *= self.hyp['cls']
        bs = tobj.shape[0]  # batch size
        css_loss=self.BCEcls(pre_css,css_label)
        return (lbox + lobj + lcls) * bs, torch.cat((lbox, lobj, lcls)).detach(), css_loss
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        bs_aux, as_aux_, gjs_aux, gis_aux, targets_aux, anchors_aux = self.build_targets2(p[3:self.nl], targets, imgs)
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p[:3], targets, imgs)
        pre_gen_gains_aux = [torch.tensor(pp.shape, device=device)[[3, 2, 3, 2]] for pp in p[3:self.nl]]
        pre_gen_gains = [torch.tensor(pp.shape, device=device)[[3, 2, 3, 2]] for pp in p[:3]]

        # Losses       any self.nf into self.nf-3
        for i in range(self.nl-3):  # layer index, layer predictions
            pi = p[i]
            pi_aux = p[i + self.na]
            ...
            if n:
                ...
                # Objectness
                tobj[b, a, gj, gi] = ((1.0 - self.gr) + self.gr * iou.detach().clamp(0).type(tobj.dtype)).reshape(len(iou))  # iou ratio
            ...
            if n_aux:
                ...
                lbox += 0.1 * (1.0 - iou_aux).mean()  # iou loss    0.25->0.15

                # Objectness
                tobj_aux[b_aux, a_aux, gj_aux, gi_aux] = ((1.0 - self.gr) + self.gr * iou_aux.detach().clamp(0).type(
                    tobj_aux.dtype)).reshape(len(iou_aux))  # iou ratio
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    lcls += 0.1 * self.BCEcls(ps_aux[:, 5:], t_aux)  # BCE     0.25

            obji = self.BCEobj(pi[..., 4], tobj)
            obji_aux = self.BCEobj(pi_aux[..., 4], tobj_aux)
            lobj += obji * self.balance[i] + 0.1 * obji_aux * self.balance[i]  # obj loss   0.25
            lobj += obji * self.balance[i]  # obj loss
