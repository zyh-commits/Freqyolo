...
# def cluster_nms( boxes, scores, NMS_threshold:float=0.5):
#     '''
#     Arguments:
#         boxes (Tensor[N, 4])
#         scores (Tensor[N, 1])
...
def my_soft_nms(bboxes, scores, iou_thresh=0.5, sigma=0.5, score_threshold=0.25):
    bboxes = bboxes.contiguous()              #      0.5                   0.25
    ...
    while order.numel() > 0:
        ...
        ##DIOU
        eps = 1e-9
        rho2 = (( x1[order[1:]] + x2[order[1:]] - x1[i] - x2[i]) ** 2 +
                    (y1[order[1:]] + y2[order[1:]] - y1[i] - y2[i]) ** 2) / 4  # center distance squared
        c2 =  (xx2 - xx1).clamp(min=0) ** 2 +(yy2 - yy1).clamp(min=0) ** 2 + eps  # convex diagonal squared
        iou= iou - rho2 / c2
        #
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # i = NMS(boxes, scores, iou_thres, GIoU=False, DIoU=False, CIoU=True)
        i = my_soft_nms(boxes, scores, iou_thres)
        # print('using soft nms')
        # i = cp_cluster(boxes, scores, iou_thres)
        # i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS    官方的非极大抑制
        # print('using originasl nms')
        #
